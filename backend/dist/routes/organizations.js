...
router.get('/bodies', auth_1.requireAuth, async (req, res) => {
    const bodies = await (0, db_1.query)(`
...
router.get('/', auth_1.requireAuth, async (req, res) => {
    ...
    const bodyQuery = req.query.body;
    const conditions = [];
    const params = [];
...
async function handleGetTree(req, res, targetBodyParam) {
    const user = req.userSession;
    const bodyQuery = (targetBodyParam || req.query.body || '').toUpperCase();
    const conditions = [];
    const params = [];
...
router.get('/tree', auth_1.requireAuth, (req, res) => handleGetTree(req, res));
...
router.get('/police/tree', auth_1.requireAuth, (req, res) => handleGetTree(req, res, 'POLICE'));
router.get('/judiciary/tree', auth_1.requireAuth, (req, res) => handleGetTree(req, res, 'JUDICIARY'));
router.get('/forensics/tree', auth_1.requireAuth, (req, res) => handleGetTree(req, res, 'FORENSICS'));
router.get('/master/tree', auth_1.requireAuth, (req, res) => handleGetTree(req, res, 'MASTER'));
// GET /api/organizations/nodes/:id or /:id (Node Details Inspector)
async function handleGetNodeDetails(req, res) {
    const user = req.userSession;
    const nodeId = req.params.id;
    const nodeRes = await (0, db_1.query)(`
    SELECT o.id, o.parent_id, o.body_id, o.agency_branch, o.type_id, o.node_type_id, o.name, o.code,
  ...
  `, [nodeId]);
    ...
    const childrenRes = await (0, db_1.query)(`
  ...
  `, [nodeId]);
    ...
    const adminsRes = await (0, db_1.query)(`
  ...
  `, [nodeId]);
    ...
    const isMaster = user.roleId === 'MASTER_ADMIN' || user.roleId === 'SYSTEM_MASTER_ADMIN';
    const isWithinScope = isMaster || (node.agency_branch === user.agencyBranch &&
...
router.get('/nodes/:id', auth_1.requireAuth, handleGetNodeDetails);
router.get('/:id', auth_1.requireAuth, handleGetNodeDetails);
// Helper for updating node
async function handleUpdateNode(req, res) {
    const user = req.userSession;
    const nodeId = req.params.id;
    const { name, jurisdictionArea, metadata } = req.body;
    const nodeRes = await (0, db_1.query)(`SELECT * FROM organization_nodes WHERE id = $1;`, [nodeId]);
    if (nodeRes.rows.length === 0) {
        return res.status(404).json({ error: 'Not Found', message: 'Organization node not found' });
    }
    const node = nodeRes.rows[0];
    const authz = await (0, authorization_1.authorize)(user, 'ORG_UPDATE', { type: 'ORG', owningOrgId: nodeId }, {
        ip: req.ip,
...
router.put('/nodes/:id', auth_1.requireAuth, handleUpdateNode);
router.put('/:id', auth_1.requireAuth, handleUpdateNode);
// Helper for updating node status / disabling
async function handleUpdateNodeStatus(req, res) {
    const user = req.userSession;
    const nodeId = req.params.id;
...
router.put('/nodes/:id/status', auth_1.requireAuth, handleUpdateNodeStatus);
router.put('/:id/status', auth_1.requireAuth, handleUpdateNodeStatus);
...
async function handleCreateNode(req, res) {
    ...
    // Parent must exist
    const parentRes = await (0, db_1.query)(`
  ...
  `, [parentId]);
    ...
    // Authorize creation
    const authz = await (0, authorization_1.authorize)(user, 'ORG_CREATE', { type: 'ORG', owningOrgId: parentId }, {
        ip: req.ip,
        userAgent: req.headers['user-agent'],
    });
    if (!authz.allowed) {
    ...
    try {
        const newOrg = await (0, db_1.query)(`
...
router.post('/', auth_1.requireAuth, handleCreateNode);
// POST /api/organizations/nodes (alias)
router.post('/nodes', auth_1.requireAuth, handleCreateNode);
