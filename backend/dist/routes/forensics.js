...
router.get('/cases/:id/forensics', auth_1.requireAuth, async (req, res) => {
    ...
    const submissions = await (0, db_1.query)(`
    SELECT fs.*,
...
router.post('/cases/:id/forensics/submit', auth_1.requireAuth, async (req, res) => {
    const user = req.userSession;
    const caseId = req.params.id;
    ...
    if (caseRes.rows.length === 0)
        return res.status(404).json({ error: 'Not Found', message: 'Case not found' });
    const authz = await (0, authorization_1.authorize)(user, 'FORENSIC_CREATE', {
        type: 'FORENSIC',
        caseId,
        owningOrgId: caseRes.rows[0].originating_organization_id,
    }, { ip: req.ip, userAgent: req.headers['user-agent'] });
...
router.get('/submissions', auth_1.requireAuth, async (req, res) => {
    const user = req.userSession;
    const authz = await (0, authorization_1.authorize)(user, 'FORENSIC_EXAMINE', { type: 'FORENSIC' }, {
        ip: req.ip,
    ...
    if (user.roleId === 'MASTER_ADMIN') {
        submissionsRes = await (0, db_1.query)(`
...
router.post('/submissions/:id/reports', auth_1.requireAuth, async (req, res) => {
    const user = req.userSession;
  ...
  `, [submissionId]);
    ...
    const authz = await (0, authorization_1.authorize)(user, 'FORENSIC_REPORT', {
        type: 'FORENSIC',
        id: submissionId,
        caseId: sub.case_id,
    ...
    try {
        const reportRes = await (0, db_1.query)(`
    ...
    `, [submissionId, reportNumber, user.userId, summaryOfAnalysis, formalConclusion]);
        ...
        await (0, db_1.query)(`UPDATE forensic_submissions SET status = 'REPORT_ISSUED' WHERE id = $1;`, [submissionId]);
