...
router.get('/', auth_1.requireAuth, async (req, res) => {
  ...
  `;
    const params = [user.userId, user.roleId, user.organizationPath, user.organizationId];
    if (status) {
        params.push(status);
    ...
    const casesRes = await (0, db_1.query)(queryText, params);
    return res.json({ cases: casesRes.rows });
});
// POST /api/cases (New FIR or Legacy Case Creation)
router.post('/', auth_1.requireAuth, async (req, res) => {
    const user = req.userSession;
    const { caseType, // 'NEW_FIR' | 'LEGACY_FIR'
    title, description, incidentDate, incidentLocation, originatingOrganizationId, leadInvestigatorId, legacyFirNumber, firData, confirmDuplicateOverride, } = req.body;
    const targetOrgId = originatingOrganizationId || user.organizationId;
    // 1. Authorize Case Creation
    const authz = await (0, authorization_1.authorize)(user, 'CASE_CREATE', {
        type: 'CASE',
    ...
    const orgRes = await (0, db_1.query)(`SELECT code, name FROM organization_nodes WHERE id = $1;`, [targetOrgId]);
    ...
    if (caseType === 'LEGACY_FIR') {
        ...
        const dupRes = await (0, db_1.query)(`
...
router.get('/:id', auth_1.requireAuth, async (req, res) => {
  ...
  `, [caseId]);
    ...
    // Load FIR Data
    const firRes = await (0, db_1.query)(`SELECT * FROM firs WHERE case_id = $1;`, [caseId]);
...
router.put('/:id/status', auth_1.requireAuth, async (req, res) => {
    ...
    if (caseRes.rows.length === 0)
        return res.status(404).json({ error: 'Not Found', message: 'Case not found' });
    const authz = await (0, authorization_1.authorize)(user, 'CASE_UPDATE', {
        type: 'CASE',
        id: caseId,
    ...
    await (0, db_1.query)(`UPDATE cases SET status = $1, updated_at = NOW() WHERE id = $2;`, [status, caseId]);
    await (0, db_1.query)(`
    INSERT INTO case_timeline (case_id, event_type, title, description, actor_id, organization_id, entity_type, entity_id)
