...
router.get('/cases/:id/court', auth_1.requireAuth, async (req, res) => {
    ...
    const courtCaseRes = await (0, db_1.query)(`
...
router.post('/cases/:id/court', auth_1.requireAuth, async (req, res) => {
    const user = req.userSession;
    const caseId = req.params.id;
    ...
    if (caseRes.rows.length === 0)
        return res.status(404).json({ error: 'Not Found', message: 'Case not found' });
    const authz = await (0, authorization_1.authorize)(user, 'COURT_CREATE', {
        type: 'COURT',
        caseId,
    ...
    try {
        const ccRes = await (0, db_1.query)(`
    ...
    `, [caseId, courtOrganizationId]);
        ...
        await (0, db_1.query)(`UPDATE cases SET status = 'TRIAL_IN_PROGRESS', updated_at = NOW() WHERE id = $1;`, [caseId]);
        // Timeline event
        await (0, db_1.query)(`
...
router.post('/cases/:id/hearings', auth_1.requireAuth, async (req, res) => {
    const user = req.userSession;
    ...
    const ccRes = await (0, db_1.query)(`SELECT case_id, court_organization_id FROM court_cases WHERE id = $1;`, [courtCaseId]);
    if (ccRes.rows.length === 0)
        return res.status(404).json({ error: 'Not Found', message: 'Court case not found' });
    const cc = ccRes.rows[0];
    const authz = await (0, authorization_1.authorize)(user, 'COURT_HEARING_RECORD', {
        type: 'COURT',
        id: courtCaseId,
        caseId: cc.case_id,
...
router.post('/cases/:id/orders', auth_1.requireAuth, async (req, res) => {
    const user = req.userSession;
    const courtCaseId = req.params.id;
    ...
    if (ccRes.rows.length === 0)
        return res.status(404).json({ error: 'Not Found', message: 'Court case not found' });
    const cc = ccRes.rows[0];
    const authz = await (0, authorization_1.authorize)(user, 'COURT_ORDER_ISSUE', {
        type: 'COURT',
        id: courtCaseId,
        caseId: cc.case_id,
        owningOrgId: cc.court_organization_id,
...
router.post('/cases/:id/judgement', auth_1.requireAuth, async (req, res) => {
    const user = req.userSession;
    const courtCaseId = req.params.id;
    ...
    if (ccRes.rows.length === 0)
        return res.status(404).json({ error: 'Not Found', message: 'Court case not found' });
    const cc = ccRes.rows[0];
    const authz = await (0, authorization_1.authorize)(user, 'COURT_JUDGEMENT', {
        type: 'COURT',
        id: courtCaseId,
        caseId: cc.case_id,
        owningOrgId: cc.court_organization_id,
  ...
  `, [courtCaseId, judgementDate || new Date(), verdict, JSON.stringify(sentencesAwarded || []), judgementSummary, user.userId]);
    ...
    await (0, db_1.query)(`UPDATE court_cases SET status = 'DISPOSED', current_stage = 'JUDGEMENT', updated_at = NOW() WHERE id = $1;`, [courtCaseId]);
    await (0, db_1.query)(`UPDATE cases SET status = 'DISPOSED', updated_at = NOW() WHERE id = $1;`, [cc.case_id]);
    await (0, db_1.query)(`
    INSERT INTO case_timeline (case_id, event_type, title, description, actor_id, organization_id, entity_type, entity_id)
