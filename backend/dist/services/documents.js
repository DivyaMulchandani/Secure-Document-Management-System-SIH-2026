 ...
 */
async function scanForMalware(_buffer) {
...
async function validateUpload(buffer, originalFilename) {
    ...
    const ext = path_1.default.extname(originalFilename).toLowerCase().replace('.', '');
...
async function storeDocument(caseId, title, category, classification, fileBuffer, originalFilename, mimeType, user, ip, userAgent) {
    ...
    try {
        ...
        return await (0, db_1.withTransaction)(async (tx) => {
...
async function getDocumentStream(documentId) {
  ...
  `, [documentId]);
    ...
    const actualHash = crypto_1.default.createHash('sha256').update(fs_1.default.readFileSync(physicalPath)).digest('hex');
