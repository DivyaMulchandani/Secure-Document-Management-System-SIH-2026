...
export interface ResourceTarget {
  type: 'CASE' | 'FIR' | 'INVESTIGATION' | 'EVIDENCE' | 'FORENSIC' | 'COURT' | 'DOCUMENT' | 'USER' | 'ORG' | 'SYSTEM';
  id?: any;
  owningOrgId?: any;
  caseId?: any;
  targetOrgId?: any;
...
const AGENCY_BOUNDARY_ACTIONS: Record<string, Set<string>> = {
  POLICE: new Set([
...
export async function authorize(
  ...
  if (targetOrgId) {
    ...
    const userScopesRes = await query(`
    ...
    if (resource.caseId) {
      const activeDelegRes = await query(`
        SELECT id FROM delegated_access
        WHERE case_id = $1 AND granted_to_user_id = $2
          AND status = 'ACTIVE'
          AND NOW() BETWEEN starts_at AND expires_at;
    ...
    const targetBranchRes = await query(`
