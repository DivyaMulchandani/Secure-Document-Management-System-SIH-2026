...
import { query, initDatabase, pool } from './db';
...
export async function seedDatabase(): Promise<void> {
  ...
  await query(`DELETE FROM admin_scopes;`);
  ...
  await query(`DELETE FROM organization_nodes;`);
  ...
  const suratCommRes = await query(`
    INSERT INTO organization_nodes (
      parent_id, body_id, agency_branch, type_id, name, code, hierarchy_path, level, jurisdiction_area
    ) VALUES (
  ...
  let rfslSuratId: string = '';
  let rfslAmdId: string = '';
  for (const r of rfslNodes) {
    ...
    if (r.code === 'GUJ-RFSL-SURAT') rfslSuratId = res.rows[0].id;
    if (r.code === 'GUJ-RFSL-AHMEDABAD') rfslAmdId = res.rows[0].id;
  }
  ...
  await query(`UPDATE organization_nodes SET node_type_id = type_id WHERE node_type_id IS NULL;`);
  ...
  const masterPasswordHash = await bcrypt.hash('Gov@Secure2026!', 10);
