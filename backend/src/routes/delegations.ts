...
router.post('/', requireAuth, async (req: Request, res: Response) => {
  ...
  const authz = await authorize(user, 'DELEGATION_MANAGE', {
    type: 'CASE',
    id: caseId,
    caseId,
...
router.put('/:id/revoke', requireAuth, async (req: Request, res: Response) => {
  const user = req.userSession!;
  const delegId = req.params.id;

  const dRes = await query(`SELECT * FROM delegated_access WHERE id = $1;`, [delegId]);
  if (dRes.rows.length === 0) return res.status(404).json({ error: 'Not Found', message: 'Delegation not found' });
  const d = dRes.rows[0];
  ...
  await query(`UPDATE delegated_access SET status = 'REVOKED' WHERE id = $1;`, [delegId]);
