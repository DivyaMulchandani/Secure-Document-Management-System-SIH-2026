...
router.get('/:id/investigation', requireAuth, async (req: Request, res: Response) => {
  ...
  const authz = await authorize(user, 'INVESTIGATION_READ', {
    type: 'INVESTIGATION',
    caseId,
...
router.post('/:id/persons', requireAuth, async (req: Request, res: Response) => {
  const user = req.userSession!;
  const caseId = req.params.id;
  ...
  const authz = await authorize(user, 'INVESTIGATION_CREATE', {
    type: 'INVESTIGATION',
    caseId,
    owningOrgId: caseRes.rows[0].originating_organization_id,
  }, { ip: req.ip, userAgent: req.headers['user-agent'] });
...
router.post('/:id/diary', requireAuth, async (req: Request, res: Response) => {
  const user = req.userSession!;
  const caseId = req.params.id;
  const caseRes = await query(`SELECT originating_organization_id FROM cases WHERE id = $1;`, [caseId]);
  if (caseRes.rows.length === 0) return res.status(404).json({ error: 'Not Found', message: 'Case not found' });
  ...
  const countRes = await query(`SELECT COUNT(*) as count FROM investigations WHERE case_id = $1;`, [caseId]);
  const entryNum = parseInt(countRes.rows[0].count, 10) + 1;

  const diaryRes = await query(`
...
router.post('/:id/panchnama', requireAuth, async (req: Request, res: Response) => {
  const user = req.userSession!;
  const caseId = req.params.id;
...
router.post('/:id/witnesses', requireAuth, async (req: Request, res: Response) => {
  const user = req.userSession!;
  const caseId = req.params.id;
...
router.post('/:id/chargesheet', requireAuth, async (req: Request, res: Response) => {
  const user = req.userSession!;
  const caseId = req.params.id;
  ...
  const authz = await authorize(user, 'INVESTIGATION_UPDATE', {
    type: 'INVESTIGATION',
    caseId,
    owningOrgId: caseRes.rows[0].originating_organization_id,
  }, { ip: req.ip, userAgent: req.headers['user-agent'] });
  ...
  await query(`UPDATE cases SET status = 'CHARGESHEETED', updated_at = NOW() WHERE id = $1;`, [caseId]);
